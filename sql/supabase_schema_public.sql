-- ===== SCHEMA SQL PARA TABELAS PÚBLICAS =====
-- Este script cria apenas as tabelas públicas, sem referências ao schema auth
...
-- Verificar e criar tabela de Usuários (sem referência ao auth.users)
DO $$
BEGIN
...
-- Verificar e criar tabela de Auditoria (sem referência ao auth.users)
DO $$
BEGIN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'movements' AND column_name = 'created_by') THEN
        ALTER TABLE movements ADD COLUMN created_by UUID;
    ...
    IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'custom_types' AND column_name = 'created_by') THEN
        ALTER TABLE custom_types ADD COLUMN created_by UUID;
...
-- Criar políticas RLS básicas (sem referência ao auth)
DO $$
BEGIN
    -- Políticas para users (acesso público temporário para criação de admin)
    IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Enable all access for users') THEN
        CREATE POLICY "Enable all access for users" ON users FOR ALL USING (true);
        RAISE NOTICE 'Política Enable all access for users criada';
    END IF;
    
    -- Políticas para audit_log (acesso público temporário)
    IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Enable all access for audit_log') THEN
        CREATE POLICY "Enable all access for audit_log" ON audit_log FOR ALL USING (true);
        RAISE NOTICE 'Política Enable all access for audit_log criada';
    END IF;
    
    -- Políticas para products (acesso público temporário)
    IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Enable all access for products') THEN
        ...
        RAISE NOTICE 'Política Enable all access for products criada';
    END IF;
    
    -- Políticas para movements (acesso público temporário)
    IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Enable all access for movements') THEN
        DROP POLICY IF EXISTS "Enable all access for now" ON movements;
        CREATE POLICY "Enable all access for movements" ON movements FOR ALL USING (true);
        RAISE NOTICE 'Política Enable all access for movements criada';
    END IF;
    
    -- Políticas para custom_types (acesso público temporário)
    IF NOT EXISTS (SELECT FROM pg_policies WHERE policyname = 'Enable all access for custom_types') THEN
        DROP POLICY IF EXISTS "Enable all access for now" ON custom_types;
        CREATE POLICY "Enable all access for custom_types" ON custom_types FOR ALL USING (true);
        RAISE NOTICE 'Política Enable all access for custom_types criada';
    END IF;
END $$;
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'SCHEMA PÚBLICO EXECUTADO COM SUCESSO!';
    RAISE NOTICE 'Todas as tabelas foram criadas';
    RAISE NOTICE 'Políticas temporárias habilitadas para criação de admin';
    RAISE NOTICE '========================================';
END $$;
